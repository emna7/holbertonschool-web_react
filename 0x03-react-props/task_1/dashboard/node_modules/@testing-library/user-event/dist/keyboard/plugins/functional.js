...
var _calculateBackspaceValue = require("./functional/calculateBackspaceValue");
...
const keydownBehavior = [{
  ...
  handle: (keyDef, element, options, state) => {
    const {
      newValue,
      newSelectionStart
    } = (0, _calculateBackspaceValue.calculateNewBackspaceValue)(element, state.carryValue);
    ...
    if (state.carryValue) {
      state.carryValue = (0, _utils.getValue)(element) === newValue ? undefined : newValue;
    }
  }
}];
...
const keypressBehavior = [{
  ...
  (0, _utils.isInstanceOfElement)(element, 'HTMLAnchorElement') && Boolean(element.href)),
  handle: (keyDef, element, options, state) => {
    _dom.fireEvent.click(element, (0, _getEventProps.getMouseEventProps)(state));
  }
}, {
  matches: (keyDef, element) => keyDef.key === 'Enter' && (0, _utils.isInstanceOfElement)(element, 'HTMLInputElement'),
